...
def _approx_dict(result, expected):
    """Assert all key-value pairs in expected match result within floating-point tolerance."""
    for k, v in expected.items():
        assert result[k] == pytest.approx(v, rel=1e-9), f"Mismatch at key {k}"


def test_value_iteration():
    _approx_dict(value_iteration(sequential_decision_environment, .01), {
        (3, 2): 1.0, (3, 1): -1.0,
    ...
    _approx_dict(value_iteration(sequential_decision_environment_1, .01), {
        (3, 2): 1.0, (3, 1): -1.0,
    ...
    _approx_dict(value_iteration(sequential_decision_environment_2, .01), {
        (3, 2): 1.0, (3, 1): -1.0,
    ...
    _approx_dict(value_iteration(sequential_decision_environment_3, .01), {
