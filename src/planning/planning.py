...
from ..csp.csp import sat_up, NaryCSP, Constraint, ac_search_solver, is_constraint
from ..logic.logic import FolKB, conjuncts, unify_mm, associate, SAT_plan, cdcl_satisfiable
from ..search.search import Node
from ..shared.utils import Expr, expr, first
...
class ForwardPlan(search.Problem):
    ...
    def h(self, state):
        ...
        relaxed_planning_problem = PlanningProblem(
            initial=state.state,
            goals=self.goal,
            actions=[action.relaxed() for action in self.planning_problem.actions])
        try:
