...
- Q: Does one aircraft arrival generate demand across multiple consecutive slots? → A: Yes — demand spans the full arrival window per aircraft type (narrow-body 1 h, wide-body 1 h, cargo 2 h). *(Window defaults updated 2026-05-04: calibrated to EFHK reference data; original estimates were wide-body 2 h, cargo 3 h.)*
...
- Q: Are departure window durations the same as arrival window durations? → A: Default to the same durations (narrow-body 1 h, wide-body 1 h, cargo 2 h); configurable independently per run. *(Window defaults updated 2026-05-04; see Session 2026-05-04 for rationale.)*
...
- Q: Should `arrival_capacity_per_worker` and `departure_capacity_per_worker` share a single value per aircraft type, or be configured independently per direction? → A: Separate fields — `arrival_capacity_per_worker` and `departure_capacity_per_worker` — each a `dict[AircraftType, float]` in `DemandConfig`, configured independently. Default 1.0 for all types (identity).
- Q: What drove the arrival/departure window defaults change (wide-body 2 h → 1 h, cargo 3 h → 2 h)? → A: Calibrated to the EFHK reference dataset — observed actual handling durations at Helsinki-Vantaa are shorter than the original estimates; the updated defaults better reflect measured ground-handling times for each category at this airport.
- Q: What drove the `shift_length` default change from 8 h to 10 h? → A: Matches the standard EFHK/Finavia ground-handling shift contract length of 10 hours.
...
- **FR-016**: System MUST accept per-direction, per-aircraft-type worker capacity parameters (`arrival_capacity_per_worker` and `departure_capacity_per_worker` in `DemandConfig`) that express the number of flights one worker can handle per hour. Each parameter is a `dict[AircraftType, float]` with default `1.0` for all types (identity — no effect on demand). When a value > 1.0 is set, the effective staffing standard for that direction and type is divided by the capacity value before computing demand. The two directions are configured independently; a different capacity may apply to arrival handling versus departure preparation for the same aircraft type.
...
- **Aircraft Type**: A category of aircraft — one of narrow-body, wide-body, or cargo — defined by its staffing standard (the number of workers required per flight of that type), its arrival window duration, its departure window duration, and optionally its per-direction worker capacity (how many flights of this type a single worker can handle per hour during arrival handling or departure preparation).
...
- Shift length is uniform for all workers within a single run; the default is 10 hours (matching the standard EFHK/Finavia ground-handling shift contract), configurable per run.
...
- Arrival window defaults per aircraft category: narrow-body 1 hour, wide-body 1 hour, cargo 2 hours. These values are configurable per run and cover arrival handling only. They were calibrated against the EFHK reference dataset; earlier estimates (wide-body 2 h, cargo 3 h) overstated handling durations. A flight arriving outside the tolerance window occupies demand starting from its actual arrival slot for the full arrival window duration of its category.
- The departure window per aircraft category covers departure preparation only (fuelling, boarding prep, final checks before pushback). Default durations match arrival window values (narrow-body 1 hour, wide-body 1 hour, cargo 2 hours); configurable independently per run. When a departure's preparation window extends before the operating day start, the window is clipped silently to the day start boundary; no error is raised and no demand is attributed to pre-day slots.
- Worker capacity per aircraft type is configurable independently for arrivals (`arrival_capacity_per_worker`) and departures (`departure_capacity_per_worker`) in `DemandConfig`. Each is a `dict[AircraftType, float]` with default `1.0` (one flight per worker — no scaling). When capacity > 1.0 for a type, the effective staffing standard for that direction is divided by the capacity value before demand is computed. The two capacity parameters are independent of each other and of the staffing standards.
